...
GITHUB_REPO_NAME                := ansible-role-oh-my-zsh
IMAGE_BASE                      := $(GITHUB_ORG_NAME)/$(GITHUB_REPO_NAME)
IMAGE_TAG                       := $(IMAGE_BASE):$(PR_SHA)

# Build configuration

role_name                 := ansible-role-oh-my-zsh
...
# Docker run command
###############################################################################################################3
DOCKER_RUN=time docker run --rm -e "PLATFORM_NAME=molecule-test-$(PR_SHA)" -e "GIT_SHA=$(PR_SHA)" -e "_DATE=$(_DATE)" -e "TERM=xterm" -w /tmp/ansible-role-oh-my-zsh
...
DOCKER_RUN+= -v "$(CURRENT_DIR):/tmp/ansible-role-oh-my-zsh:rw"
...
.PHONY: run
run:
	ansible-playbook --ask-become-pass -i "localhost," -c local playbook.yml --extra-vars="bossjones__oh__my__zsh__user=$(WHOAMI)"
